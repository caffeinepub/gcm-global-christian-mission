...
/**
 * Create a properly configured HttpAgent using createSync for v3 response support.
 */
function createAgent(options?: { host?: string }): HttpAgent {
  return HttpAgent.createSync({ host: options?.host });
}

/**
 * Create a StorageClient with a properly initialized agent.
 * Use this for media uploads in pages instead of creating a raw HttpAgent.
 */
export async function createStorageClientForUpload(): Promise<StorageClient> {
  const config = await loadConfig();
  const agent = createAgent({ host: config.backend_host });
